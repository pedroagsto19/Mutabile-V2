...
export function GanttChart({ project }: GanttChartProps) {
  ...
  const getActivityPosition = (startDate: Date, endDate: Date) => {
    const activityStart = differenceInDays(startDate, startDate);
                  ...
                  <div key={activity.id} className="flex items-center">
                    ...
                    <div className="flex-1 relative h-12 bg-gray-50 rounded">
                      ...
                      {(showMode === 'planned' || showMode === 'both') && (
                        <div
                          className={`absolute rounded opacity-60 ${getStatusColor(activity, false)}`}
                          style={{
                          ...
                          title={`Previsto: ${format(activity.planned.startDate, 'dd/MM/yyyy')} - ${format(activity.planned.endDate, 'dd/MM/yyyy')}`}
                        >
                          {showMode === 'planned' && (
                      ...
                      {(showMode === 'actual' || showMode === 'both') && (
                        <div
                          className={`absolute rounded ${getStatusColor(activity, true)}`}
                          style={{
                          ...
                          title={`Real: ${format(activity.actual.startDate || activity.planned.startDate, 'dd/MM/yyyy')} - ${format(activity.actual.endDate || activity.actual.estimatedEndDate || activity.planned.endDate, 'dd/MM/yyyy')}`}
                        >
