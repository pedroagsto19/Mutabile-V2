...
class SupplierStorage {
  ...
  static clearAllData() {
    console.log('Limpando todos os dados de fornecedores...');
    try {
      localStorage.removeItem(this.SUPPLIERS_KEY);
      console.log('Dados de fornecedores limpos com sucesso!');
    } catch (error) {
      console.error('Erro ao limpar dados de fornecedores:', error);
    }
  }
